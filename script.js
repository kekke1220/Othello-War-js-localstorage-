 ...
 * アプリケーション設定
 *****************************************/
// トースター(画面隅に現れる長方形の通知メッセージのこと)のオプション設定
toastr.options = {
    tapToDismiss: false,
    timeOut: 0,
    extendedTimeOut: 0,
...
function initializeEvent() {
    // ターン表示を削除する
    toastr.remove();
    //ゲーム途中でリセットボタンを押す➡︎表示されていたトーストを削除
...
function toastEndMessage(message) {
    ...
    if (countBlack == countWhite) {
        ...
        toastr.success(message + "<br/>" + judgeString + "whiteの勝利!");
    } else {
        toastr.success(message + "<br/>" + judgeString + "blackの勝利!");
    }
}
...
$(document).ready(function() {
    $("#btn3").click(function() {
        var audioElement = $("#audio3")[0];
        audioElement.play();
    });
});

// 勝利数、敗北数、引き分け数の初期値を設定
...
$('#btn-draw').click(function() {
    var draws = parseInt(localStorage.getItem('draws'));
